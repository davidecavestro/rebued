	...
	while (true) {
            ...
            if (line.length() > 0) {
                ...
                if ((firstChar != '#') && (firstChar != '!')) {
                    ...
                    put(key, value);
					initialOrder.add (key);
					if (nextComment.length ()>0){
						commentsMap.put (key, nextComment.toString ());
...
//                                           "Malformed \\uxxxx encoding.");
	...
	public synchronized void setComment (String key, String comment) {
		if (comment!=null){
			commentsMap.put (key, comment);
		} else {
			commentsMap.remove (key);
	...
	public synchronized String getComment (String key) {
		return (String)commentsMap.get (key);
	}
	
	private final List initialOrder = new ArrayList ();
